...
resource "aws_iam_role_policy_attachment" "vault_kms_auto_unseal" {
  role       = aws_iam_role.vault_eks_server_role.name
  policy_arn = aws_iam_policy.vault_kms_auto_unseal_policy.arn
}

resource "aws_iam_role_policy_attachment" "vault_aws_auth" {
  role       = aws_iam_role.vault_eks_server_role.name
  policy_arn = aws_iam_policy.vault_aws_auth_policy.arn
}

resource "aws_iam_role_policy_attachment" "vault_aws_secret" {
  role       = aws_iam_role.vault_eks_server_role.name
  policy_arn = aws_iam_policy.vault_aws_secret_policy.arn
}